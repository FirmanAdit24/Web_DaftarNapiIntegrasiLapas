...
onAuthStateChanged(auth, (user) =>{
   if(user){
      const path = "daftarStatusNapi/NapiPengajuanIntegrasi"
...
function insertData(dbRef){
  ...
  for(var i = 0 ; i < insertTable.rows.length; i++){
   ...
   insertMultiData(newRef, "insertNama"+i, "insertPerkara"+i, "insertPidana"+i, "insertKet"+i, i )
   
  }
  console.log(insertTable.rows.length)

  function insertMultiData(newRef, namaId, perkaraId,pidanaId ,ketId, i){
    if(document.getElementById(namaId).value != "" &&  document.getElementById(perkaraId).value  != ""  &&  document.getElementById(pidanaId).value  != ""  &&  document.getElementById(ketId).value  != ""){

      set(newRef, {
       nama: document.getElementById(namaId).value,
       perkara: document.getElementById(perkaraId).value,
       pidana:document.getElementById(pidanaId).value,
...
document.getElementById('confirmResetBtn').addEventListener('click', function() {
  remove(ref(dbs, "daftarStatusNapi/NapiPengajuanIntegrasi"))
